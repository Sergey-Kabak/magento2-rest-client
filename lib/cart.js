...
module.exports = function (restClient) {
    ...
    module.update = function (customerToken, cartId, cartItem, adminRequest = false) {
        if (adminRequest) {
            return restClient.post('/carts/' + cartId + '/items-and-get-totals/', { cartItem: cartItem });
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.post('/carts/mine/items-and-get-totals', { cartItem: cartItem }, customerToken);
            } else.
            {
                return restClient.post('/guest-carts/' + cartId + '/items-and-get-totals', { cartItem: cartItem });
            }
        }
    }
